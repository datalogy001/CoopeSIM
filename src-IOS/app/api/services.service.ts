...
validate_voucher_code(obj: any, access_token:any) {
  ...
  getBundleFromDB(obj: any) {
    this.authToken =  window.localStorage.getItem('coop_auth_token') != null? window.localStorage.getItem('coop_auth_token') : '';
    return new Promise((resolve, reject) => {
 ...
 listOfCountriesForResidence() {
    this.countryObj.language = window.localStorage.getItem('coop_language') == null ? 'en' : window.localStorage.getItem('coop_language');
    return new Promise((resolve, reject) => {
      const headers = new HttpHeaders()
        .set('Content-Type', 'application/json; charset=utf-8')
        .set('whitelabel', this.whiteLabelId)
