...
export class UpdateProfilePage implements OnInit, OnDestroy {
  ...
  private initializeCountryCodeFromStorage(): void {
    const storedPhoneCode = window.localStorage.getItem('coop_phone_code');
  ...
  private loadProfileFromStorage(): void {
    const raw = window.localStorage.getItem('coop_userDetails');
    try {
  ...
  async submit(): Promise<void> {
    ...
    try {
      ...
      if (res?.code === 200 && Array.isArray(res.data)) {
        ...
        window.localStorage.setItem('coop_userDetails', JSON.stringify(userData));
        window.localStorage.setItem('coop_auth_token', data0.token || '');
        window.localStorage.setItem('coop_loginType', 'normal');
        window.localStorage.setItem('coop_emailSettings', data0.promotion_email || '');
        window.localStorage.setItem('coop_promoSettings', data0.app_promotions || '');
        window.localStorage.setItem('coop_paymentSettings', data0.app_payment || '');
        window.localStorage.setItem('coop_serviceSettings', data0.app_service || '');
